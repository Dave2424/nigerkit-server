...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
        ...
        var defaults = {
            ...
            buttons: {
                edit: {
                    class: 'edit btn btn-info btn-link btn-sm',
                    html: '<a  type="button" rel="tooltip" title="Edit"><i class="material-icons">edit</i></a>',
                    action: 'edit'
                },
                delete: {
                    class: 'delete btn btn-danger btn-link btn-sm',
                    html: '<a type="button" rel="tooltip" title="Remove"><i class="material-icons">close</i></a>',
                    action: 'delete'
                },
                save: {
                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" data-item ="'+ $(this).text() +'" value="' + $(this).text() + '" style="display: none;" disabled>';
                            }
        ...
        var Edit = {
            ...
            submit: function(td) {
                //getting the value of the old value in the text;
                data_item = td[0].textContent;
        ...
        function ajax(action)
        {
            var serialize ='';
            if (action == 'delete') {
                serialize = $table.find('.tabledit-input').val();
                serialize = 'sku=' + serialize;
            }
            else {
                serialize = $table.find('.tabledit-input').serialize();
                serialize = 'data-item='+data_item+'&' + serialize;
            }
            if (!serialize) {
                return false;
            }
        ...
        if (settings.deleteButton) {
            ...
            $table.on('click', 'button.tabledit-delete-button', function(event) {
                if (event.handled !== true) {
                    ...
                    if (!activated) {
                        Delete.confirm($td);
                        console.log('yes it is activated');
                    }
